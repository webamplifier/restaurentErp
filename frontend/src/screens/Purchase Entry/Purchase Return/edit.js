...
export default function Edit() {
    ...
    React.useEffect(() => {
        ...
        async function fetchDetailPurchase() {
            const response = await fetch(url + 'detail/purchaseReturn/' + id, {
                headers: {
                    'Authorization': user.token
                }
            });
            ...
            if (response.ok === true) {
                ...
                if (data.status === 200) {
                    ...
                    setReturnDate(header.return_date);
                    setRemainPaymentDate(header.remainPaymentDate);
                    setCurrentParty({
    ...
    const handleSubmitItem = () => {
        if (invoiceNo && returnDate && currentParty && discountType && currentProduct && purchase_price && qty) {
            let new_item_dict = {
                item: currentProduct,
    ...
    const finalSubmit = () => {
        if (invoiceNo && returnDate && currentParty && allItems.length > 0 && totalValue) {
            setLoad(true)
            let header = {
                invoiceNo: invoiceNo,
            ...
            async function submitPurchaseReturn() {
                const response = await fetch(url + 'edit/purchaseReturn/' + id, {
                    method: 'POST',
                    headers: {
                        'Authorization': user.token
                    },
                ...
                if (response.ok === true) {
                    ...
                    setLoad(false)
                    if (data.status === 200) {
                        history.push('/#/create/purchaseReturn')
                    } else {
                        toast.error(data.message);
                    }
                }
