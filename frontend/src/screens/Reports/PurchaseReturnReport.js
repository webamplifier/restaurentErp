...
export default function PurchaseReturnReport() {
    ...
    const fields = ['#', 'invoice_number', 'party_name', 'return_date', 'bill_amount', 'paid_amount', 'remain_amount', 'status', 'action'];
            ...
            if (response.ok === true){
                ...
                if (data.status === 200){
                    ...
                    async function fetchData() {
                        const response = await fetch(url + 'purchaseReturn/report', {
                            method: 'GET',
                            headers: {
                                'Authorization': user?.token
                            }
                        ...
                        if (response.ok === true) {
                            ...
                            setLoad(false)
                            if (data.status === 200) {
                                setPurchaseReturnList(data.purchaseReturn_data);
                            } else {
    ...
    const deleteEntry = () => {
        ...
        async function deleteData() {
            const response = await fetch(url + 'delete/purchaseReturn/' + id, {
                method: 'GET',
                headers: {
                    'Authorization': user?.token
                }
    ...
    React.useEffect(() => {
        setLoad(true)
        async function fetchData() {
            const response = await fetch(url + 'get-purchase-return-list', {
                method: 'GET',
                headers: {
                    'Authorization': user?.token
                }
            ...
            if (response.ok === true) {
                ...
                setLoad(false)
                if (data.status === 200) {
                    setPurchaseReturnList(data.list);
                } else {
                    toast.error(data.message);
                }
            }
                            ...
                            scopedSlots={{
                                ...
                                'status':
                                    (item) => (
                                        <td>
                                            ...
                                            <Link to={`/edit/purchaseReturn/${item.id}`}>
                                ...
                                'action': (item) => (
                                    <td>
                                        ...
                                        <Link to={`/edit/purchaseReturn/${item.id}`}><i class="fa fa-pencil" aria-hidden="true"></i></Link>
