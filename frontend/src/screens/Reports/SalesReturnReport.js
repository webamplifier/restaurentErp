...
export default function SalesReturnReport() {
    ...
    const fields = ['#', 'invoice_number', 'customer_name', 'return_date', 'bill_amount', 'paid_amount', 'remain_amount', 'status', 'action'];
    const [salesReturnList, setSalesReturnList] = React.useState([]);
    ...
    const payBill = () => {
        ...
        async function payBillServer(){
            ...
            const response = await fetch(url + 'pay/salesReturn/' + id,{
                method : 'POST',
                headers : {
            ...
            if (response.ok === true){
                ...
                if (data.status === 200){
                    ...
                    async function fetchData() {
                        const response = await fetch(url + 'salesReturn/report', {
                            method: 'GET',
                            headers: {
                                'Authorization': user?.token
                            }
                        ...
                        if (response.ok === true) {
                            ...
                            setLoad(false)
                            if (data.status === 200) {
                                setSalesReturnList(data.salesReturn_data);
                            } else {
    ...
    const deleteEntry = () => {
        ...
        async function deleteData() {
            const response = await fetch(url + 'delete/salesReturn/' + id, {
                method: 'GET',
                headers: {
                    'Authorization': user?.token
                }
    ...
    React.useEffect(() => {
        setLoad(true)
        async function fetchData() {
            const response = await fetch(url + 'get-sales-return-list', {
                method: 'GET',
                headers: {
                    'Authorization': user?.token
                }
                            ...
                            scopedSlots={{
                                ...
                                'status':
                                    (item) => (
                                        <td>
                                            ...
                                            <Link to={`/edit/salesReturn/${item.id}`}>
                                ...
                                'action': (item) => (
                                    <td>
                                        ...
                                        <Link to={`/edit/salesReturn/${item.id}`}><i class="fa fa-pencil" aria-hidden="true"></i></Link>
