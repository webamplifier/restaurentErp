...
export default function Index(){
  const {user,setLoad} = React.useContext(userContext);
    const fields = ['#','name','category_name','action'];
    ...
    const deleteEntry = () => {
      ...
      async function deleteData(){
        const response = await fetch(url + 'deletesubcategory/' + id,{
          method : 'GET',
          headers : {
        ...
        if (data.status === 200){
          ...
          async function fetchData(){
            ...
            if (response.ok === true){
              ...
              if (data.status === 200){
                setLoad(false)
                setsubCategoryList(data.subcategory_list);
              }else{
                setLoad(false)
          ...
          <CCard>
            ...
            <CDataTable
              ...
              scopedSlots = {{
                ...
                'action': (item)=>(
                  <td>
                    <Link to={`/edit/subcategory/${item.id}`}><i class="fa fa-pencil" aria-hidden="true"></i></Link>
