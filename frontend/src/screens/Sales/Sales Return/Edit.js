...
export default function Edit() {
    ...
    React.useEffect(() => {
        ...
        async function fetchDetailSales() {
            const response = await fetch(url + 'detail/salesReturn/' + id, {
                headers: {
                    'Authorization': user.token
                }
            });
    ...
    const handleSubmitItem = () => {
        if (invoiceNo && returnDate && currentCustomer && discountType && currentProduct && qty && purchase_price) {
            let new_item_dict = {
                item: currentProduct,
    ...
    const finalSubmit = () => {
        if (invoiceNo && returnDate && currentCustomer && allItems.length > 0 && totalValue) {
            ...
            async function submitReturn() {
                const response = await fetch(url + 'edit/salesReturn/' + id, {
                    method: 'POST',
                    headers: {
                        'Authorization': user.token
                    },
                ...
                if (response.ok === true) {
                    ...
                    setLoad(false)
                    if (data.status === 200) {
                        history.push('/#/create/salesReturn')
                    } else {
                        toast.error(data.message);
                    }
                }
